...
Workflow автоматически запускается при изменении исходного кода приложения (каталог `app/`) или при изменении конфигурации Docker CI/CD.

В процессе выполнения автоматически:

- собирается Docker Image;
- публикуется в Container Registry;
